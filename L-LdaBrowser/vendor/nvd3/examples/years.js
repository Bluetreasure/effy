var topicid = parseInt(location.hash.slice(1,location.hash.length))-1;
...
d3.json("data/topic_year_data.json", function(error, data1) {
  ...
  d3.json("data/yearstwords.json", function(d2) {
    ...
    var wordData = wordData.filter(function(d3) {
      //console.log("Topic" + (topicid+1));
      return (d3.topics == "Topic" + (topicid+1));
    });
    ...
    for (var i = 0; i < 5; i++) {
      ...
      $("#wt"+(i+1)).on("click",function(){window.open("http://52.198.131.106:8080/%E8%94%A1%E8%80%81%E5%B8%AB/%E8%87%AA%E7%94%B1%E4%B8%AD%E5%9C%8B/search?q="+this.innerHTML,'_blank')});
      var svg = "<svg><g><rect x=\"0\" y=\"16\" width=\"" + parseFloat(wordData[i].probs) * 1000 + "\" height=\"13\" fill=\"#03a9f4\"></rect><text x=\"105\" y=\"27\" font-family=\"Verdana\" font-size=\"14px\"> " + (parseFloat(wordData[i].probs)*100).toFixed(2) + "% </text></g></svg>";
      ...
      .on("click",function(d){
        ...
        d3.json("data/yearstwords.json",function(d2){
          ...
          for (var i = 0 ; i < 5 ; i ++){
            ...
            var svg = "<svg><g><rect x=\"0\" y=\"16\" width=\""+(parseFloat(wordData[i].probs)*1000).toFixed(2)+"\" height=\"13\" fill=\"#03a9f4\"></rect><text x=\"105\" y=\"27\" font-family=\"Verdana\" font-size=\"14px\"> "+(parseFloat(wordData[i].probs)*100).toFixed(2)+"% </text></g></svg>";
            document.getElementById("pt"+(i+1)).innerHTML = svg;
          }
